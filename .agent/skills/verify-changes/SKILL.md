...
when_to_use: "After writing code, completing a feature, or fixing a bug. When the user says 'test this', 'verify', 'chạy thử xem', or when /verify workflow is invoked. NOT for writing new code — for proving existing code works."
...
- Prove code works by running it, not just checking it exists. Verification through execution rather than inspection. Use 
- Working on code-related tasks
- Need guidance on: Core Principle
- Need guidance on: Verification Protocol
- Need guidance on: Verification Report
- Writing or reviewing bash code


# For Node.js/TypeScript projects (server-node)
npx tsx <test-file>.ts      # Run TypeScript script directly
npm run build               # Does it compile?
npm run test                # Do tests pass?
npm run dev                 # Does it start?

# For specific AGS99 endpoints
npx tsx test-search-and-book.ts    # Test booking flow
npx tsx test-booking-real.ts       # Test real booking

# Check API responses
curl -H "Authorization: Bearer <token>" https://api.ags99.vn/v3/...
```
